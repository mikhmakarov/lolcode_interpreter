...
class ParserTest(unittest.TestCase):
    ...
    def test_expr(self):
        with open('programs/expr') as f:
            ...
            assert len(ast) == 3
            assert ast[0][0] == VISIBLE
            assert ast[0][1][0] == (YARN, 'Hai world!')
            assert ast[0][1][2][0] == SUM
            assert ast[0][1][3][0] == BOTH
            assert ast[0][1][3][1][0] is True
            assert ast[0][1][3][1][1] is False
            assert ast[0][1][4][0] == ALL
            assert ast[1][1][0][0] == SAME
            assert ast[1][1][1][0] == MAEK
            assert ast[1][1][1][1][-1] == NUMBR
